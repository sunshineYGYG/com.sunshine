...
import com.sunshine.shine.Module.AdminUser;
...
//        System.out.println(m["key"]);
    ...
    public void test29() {
        ...
        mm.computeIfAbsent("key2", v -> {
            return "val2";
        });
        mm.computeIfAbsent("key22", v -> {
            return null;
        });
        mm.computeIfAbsent("key22", v -> {
            return "val22";
        ...
        mm.computeIfAbsent("key222", v -> {
            return null;
        });
        mm.computeIfAbsent("key112", v -> {
            return "val112";
        });

        mm.computeIfAbsent("key3", v -> {
            return "val3";
        });
        mm.computeIfAbsent("key4", v -> {
            return "val4";
        });
        mm.computeIfAbsent("key5", v -> {
            return "val5";
        });
        mm.computeIfAbsent("key5", v -> {
            return "val555";
        });
        mm.computeIfAbsent("key6", v -> {
            return null;
        });
        printMap(mm);
        System.out.println();

        mm.computeIfPresent("key1", (k, v) -> {
            return "test1";
        });
//        mm.put("key1",null);
        mm.computeIfPresent("key100", (k, v) -> {
            return "Test2";
        });
        mm.computeIfPresent("key3", (k, v) -> {
            return null;
        });
        ...
        mm.compute("key55", (k, v) -> {
            return null;
        });
        mm.compute("key555", (k, v) -> {
            return "val555";
        });
        printMap(mm);
    ...
    @Test
    public void time2LocalDateTime() {
        LocalDateTime time = LocalDateTime.ofInstant(Instant.ofEpochMilli(1543463738), ZoneId.systemDefault());
        System.out.println(time);
    }

    public static List<AdminUser> mobileList = new ArrayList();
    static {
        AdminUser mAdminUser = new AdminUser();
        mAdminUser.setPhone("18610933312");
        mAdminUser.setUsername("于庭龙");
        mobileList.add(mAdminUser);

        mAdminUser = new AdminUser();
        mAdminUser.setPhone("18210119894");
        mAdminUser.setUsername("宋要辉");
        mobileList.add(mAdminUser);

        mAdminUser = new AdminUser();
        mAdminUser.setPhone("18716022536");
        mAdminUser.setUsername("赵乾泽");
        mobileList.add(mAdminUser);

        mAdminUser = new AdminUser();
        mAdminUser.setPhone("15120076052");
        mAdminUser.setUsername("高鑫磊");
        mobileList.add(mAdminUser);

        mAdminUser = new AdminUser();
        mAdminUser.setPhone("17600556713");
        mAdminUser.setUsername("杨光");
        mobileList.add(mAdminUser);

        mAdminUser = new AdminUser();
        mAdminUser.setPhone("17600346799");
        mAdminUser.setUsername("刘涛");
        mobileList.add(mAdminUser);
    }
    ...
    public void test30() {
        ...
        int i = 0;
        while (i < 100) {
            int id = new Random().nextInt(mobileList.size());
            String username = mobileList.get(id).getUsername();
//            System.out.println(username);
            i++;
            Integer sum = map.get(username);
